...
if (btn_play_song) {
  btn_play_song.addEventListener('click', () => {
    ...
    if (words) {
      const words_count = words.children.length;
      const animation_durations = words_count * 2.5 ; // in seconds

      setTimeout(() => {
        words.style.animation = `words-animation ${animation_durations}s linear infinite`;
      }, 1000);

      const reff_song_duration = 110900;
...
if (btn_send_acceptation) {
  btn_send_acceptation.addEventListener('click', () => {
    const phone_number = 6289698501592;
    const current_date = new Date();
    const formatted_current_date = new Intl.DateTimeFormat('id-ID', { dateStyle: 'full', timeStyle: 'medium' }).format(current_date);
    const text = `Hi Agus Sayangku, mulai hari ini _${formatted_current_date}_ dari pernyataan kamu, *Aku nerima kamu jadi pasangan aku.* Kita official sekarang ya sayang 😊%0a%0a love you so much`;
...
if (btn_save_date) {
  btn_save_date.addEventListener('click', () => {
    ...
    const calendar_link = `https://www.google.com/calendar/render?action=TEMPLATE&text=Anniversary Desi Maranatha Hutasoit dan Agus Marsadualan &details=To%20remember%20our%20first%20date!%0A%0AThere%E2%80%99s%20nowhere%20else%20I%E2%80%99d%20rather%20be%20than%20right%20here%20by%20your%20side%20with%20your%20hand%20in%20mine.%0A%0AEvery%20love%20story%20is%20special,%20unique,%20and%20beautiful.%20But%20ours%20is%20my%20favorite.%0A%0AAnniversary%20cheers!%20%F0%9F%A5%B3%20%20&dates=${date}/${date}&ctz=Asia/Jakarta&crm=AVAILABLE&add=agusmarsadu@gmail.com&recur=RRULE:FREQ=YEARLY`;
